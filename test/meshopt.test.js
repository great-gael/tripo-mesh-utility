/* 来自 zeux/meshoptimizer 的 21 个官方解码器测试向量，逐字照搬。它们逐个
   字节地检验比特流 —— 而这正是本模块所需要的，因为一个被错误解码的缓冲区
   不会抛出异常，而是产生一份满是凭空捏造的洞的分析报告。

   其中两个向量使用旧的索引 codec v0（头字节 0xe0），参考解码器并不认识它。
   它们没有被略去，而是被反过来使用：它们检验拒绝是否给出了解释，而不是一句
   赤裸裸的 "Assertion failed"。测试文件和 Tripo 的文件都不包含 v0。 */
var decoder = require("../lib/meshopt.js");
var assert = require("assert/strict");
var pass = 0, fail = 0;
function t(n, c, g) { c ? (pass++, console.log("PASS  " + n)) : (fail++, console.log("FAIL  " + n + " :: " + g)); }

/* 这两个用参考解码器不可能跑成绿色。 */
var NUR_V0 = { decodeIndexBuffer16: 1, decodeIndexBuffer32: 1 };
...
Object.keys(tests).forEach(function (name) {
  var fehler = null;
  try { tests[name](); } catch (e) { fehler = e; }
  if (NUR_V0[name]) {
    t(name + " lehnt v0 erklaerend ab",
      !!fehler && /old meshopt index/.test(fehler.message),
      fehler ? fehler.message : "keine Ausnahme geworfen");
  } else {
    t(name, !fehler, fehler ? fehler.message : null);
  }
});
